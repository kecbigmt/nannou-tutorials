// ref. https://github.com/nannou-org/nannou/blob/master/examples/nannou_basics/window_coordinates.rs
...
fn view(app: &App, frame: Frame) {
    ...
    if let Some(monitor) = window.current_monitor() {
        ...
        let circle = Rect::from_x_y_w_h(mouse.x, mouse.y, 5.0, 5.0);
        draw.ellipse()
            .xy(circle.xy())
            .wh(circle.wh());

        // Mouse position text.
        let text = circle.above(circle).shift_y(20.0);
